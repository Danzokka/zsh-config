...
ensure_zshrc_plugins() {
    ...
    local want_plugins=(git zsh-autosuggestions zsh-syntax-highlighting zsh-history-substring-search)
    local plugins_line="plugins=(${want_plugins[*]})"

    if [ ! -f "$zshrc" ]; then
        echo "Creating $zshrc with plugins line"
        cat > "$zshrc" <<EOF
...
EOF
    ...
    if grep -q '^plugins=' "$zshrc"; then
        ...
        # append plugins line
        printf "%s\n" "$plugins_line" >> "$zshrc"
        echo "Appended plugins line to $zshrc"
    fi
}
